...
class Order extends CI_Controller
{
        ...
        public function export_excel()
    {
        ...
        // Ambil data order tanpa filter user
        $orders = $this->Order_model->get_by_date_range($start_date, $end_date);

        // Lanjut ke proses export Excel
        $this->_generate_excel($orders, $start_date, $end_date);
    }

    /**
     * Export Excel berdasarkan tanggal + user_id
     */
    public function export_excel_id()
    {
        error_reporting(0);
        ...
        if (!$start_date || !$end_date || !$user_id) {
            show_error('Tanggal awal, akhir, dan user_id harus diisi.');
        }

        // Ambil data order berdasarkan user_id
        $orders = $this->Order_model->get_by_date_range_and_user_id($start_date, $end_date, $user_id);

        // Lanjut ke proses export Excel
        $this->_generate_excel($orders, $start_date, $end_date, $user_id);
    }

    /**
     * Private helper untuk generate file Excel (dipakai dua-duanya)
     */
    
private function _generate_excel($orders, $start_date, $end_date, $user_id = null)
{
    // Ambil data service dari database
    $rateData = $this->Master_model->get_rates();
    $serviceTypeMap = [];

    $rates = !empty($rateData) ? ($rateData['data'] ?? $rateData) : [];
    if (!empty($rates) && is_array($rates)) {
        foreach ($rates as $r) {
            $rateType = is_array($r) ? ($r['rate_type'] ?? null) : ($r->rate_type ?? null);
            $text     = is_array($r) ? ($r['text'] ?? '') : ($r->text ?? '');
    ...
    foreach ($orders as $order) {
        ...
        // === Gabungkan barang, kategori, dan jumlahkan total ===
        $barangList   = [];
        $kategoriList = [];
        $totalQty     = 0;
        $totalHarga   = 0;
        ...
        if (!empty($details)) {
            foreach ($details as $detail) {
                $barangList[]   = $detail['name'] ?? '-';
                $kategoriList[] = $detail['category'] ?? '-';
                $totalQty      += (int) ($detail['qty'] ?? 0);
                $totalHarga    += (float) ($detail['price'] ?? 0);
            }
        }

        $barangStr   = !empty($barangList) ? implode(', ', $barangList) : '-';
        $kategoriStr = !empty($kategoriList) ? implode(', ', $kategoriList) : '-';
        ...
        // Barang gabungan
        $sheet->setCellValue('P' . $row, $barangStr);
        $sheet->setCellValue('Q' . $row, $kategoriStr);
    ...
    $filename = 'Order_Detail_' . ($user_id ? "User_{$user_id}_" : '') . "{$start_date}_to_{$end_date}.xlsx";
    $writer = new Xlsx($spreadsheet);
